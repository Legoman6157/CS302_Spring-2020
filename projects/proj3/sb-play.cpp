...
std::vector<dset> sb_analyze(Superball *s) {
	...
	for (int i = 0; i < goal_pieces.size(); i++) {
		...
		while (!dfs_stack.empty()) {
			...
			dir_loc[0] = (curr_pixel.x-1) * s->c + (curr_pixel.y);
			dir_loc[1] = (curr_pixel.x) * s->c + (curr_pixel.y+1);
			dir_loc[2] = (curr_pixel.x) * s->c + (curr_pixel.y-1);
			dir_loc[3] = (curr_pixel.x+1) * s->c + (curr_pixel.y);
			...
			for (int i = 0; i < 4; i++) {
				if ( (curr_pixel.x > 0)
						...
						&& (curr_pixel.y > 0)
						&& (curr_pixel.x < s->r-1)
						&& (ds.Find(dir_loc[i]) != ds.Find(curr_pixel_loc))
						&& (s->board[dir_loc[i]] == s->board[curr_pixel_loc]) ) {

					//Match them
					ds.Union(ds.Find(dir_loc[i]), ds.Find(curr_pixel_loc));

					//Set the temporary pixel's x and y to north's
					temp_pixel.x = dir_loc[i]/s->c;
					temp_pixel.y = dir_loc[i]%s->c;
		...
		if (curr_pixel_loc == ds.Find(curr_pixel_loc) && temp_dset.pixels.size() >= 0) {
...
int main(int argc, char **argv)
{
	...
	if (dsets.size() == 0) {
		...
		if (s->empty < 5) {
			dset biggest_dset = get_biggest_dset(dsets);
